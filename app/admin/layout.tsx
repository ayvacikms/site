...
import { supabase } from "@/lib/supabase"; // Supabase istemcisi eklendi
import { 
  Users, 
  LayoutDashboard, 
  Wallet, 
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  // KRİTİK: Oturum Kontrolü
  useEffect(() => {
    const checkAdmin = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      
      if (!session) {
        // Oturum yoksa login'e fırlat
  ...
  // Yüklenme ekranı (Oturum kontrol edilirken tasarımın kaymasını önler)
  if (isLoading) {
    return (
      ...
      <main className={`flex-1 transition-all duration-300 ${isSidebarOpen ? "ml-72" : "ml-20"}`}>
        ...
        <header className="h-20 bg-white border-b border-slate-200 flex items-center justify-between px-8 sticky top-0 z-40">
          <div className="flex items-center gap-2">
            <span className="text-slate-400 font-medium capitalize">{pathname.split('/').pop() || 'Dashboard'}</span>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-[#2C3E50] font-bold border border-slate-200">
                A
              </div>
            </div>
          </div>
